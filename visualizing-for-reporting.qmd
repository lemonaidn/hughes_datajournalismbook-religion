...
One of the columns we'll need -- adherents as a percent of population -- was stored with a percent sign, so R read it as text. Let's fix that using `parse_number()`, which strips out percent signs and other non-numeric characters.
